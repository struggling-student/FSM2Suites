...
def help_tasks(c):
    ...
    print("""
Shopping App Development Tasks
==============================

Development:
  invoke start             - Start both backend and frontend servers
  invoke start-backend     - Start backend only (FastAPI on :8000)
  invoke start-frontend    - Start frontend only (React/Vite on :5173)
  invoke dev               - Alias for 'start'
  invoke stop              - Stop all running services
...
Utilities:
  invoke lint              - Run linting for both projects
  invoke build-frontend    - Build frontend for production
  invoke clean             - Clean up build artifacts

Quick start:
  invoke setup             # First time setup
  invoke start             # Start development servers
